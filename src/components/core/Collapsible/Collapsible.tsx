...
const CollapsibleTrigger = <T extends ValidComponent = "div">(props: WithStyleX<PolymorphicProps<T, CollapsibleTriggerProps<T>>>) => {
  const [local, others] = splitProps(props as WithStyleX<CollapsibleTriggerProps>, ['style'])
  return <CollapsiblePrimitive.Trigger {...stylex.attrs(styles.trigger, local.style)} {...others} />
}
...
const CollapsibleContent = <T extends ValidComponent = "div">(props: WithStyleX<PolymorphicProps<T, CollapsibleContentProps<T>>>) => {
  const [local, others] = splitProps(props as WithStyleX<CollapsibleContentProps>, ['style'])
  return <CollapsiblePrimitive.Content {...stylex.attrs(styles.triggerContent, local.style)} {...others} />
}
